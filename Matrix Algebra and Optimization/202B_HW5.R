...
f_Soft_Threshold <- function(beta,lambda,t){
  S = beta*0
  for(i in 1:dim(S)[1]){
    for(j in 1:dim(S)[2]){
      if(beta[i,j] > t*lambda){
        S[i,j] = beta[i,j] - t*lambda
      } else if(beta[i,j] < -t*lambda){
        S[i,j] = beta[i,j] + t*lambda
      } else{
        S[i,j] = 0
      } 
...
plot((1:(it+1)),f-mnist$f.star[1],type='l',xlab="Iteration",ylab="Minus Log-Likelihood",main="Proximal Gradient Descent")
grid()
