...
myAdaboost <- function(x1, x2, y){
  ...
  yp = (Yp+1)/2
  plot(x1*yp,x2*yp,main="Plot1",xlab = 'X', ylab="Y",col="blue")
  par(new=T)
  plot(x1*(-yp+1),x2*(-yp+1),main="Plot1",xlab = 'X', ylab="Y",col="red")
  
  # print(L)
  # plot(c(0,170,340,500),c(9130, 8400, 5522, 4462))
  # lines(c(0,170,340,500),c(9130, 8400, 5522, 4462))
...
myXGBoost <- function(x1, x2, y) {
  ...
  New_Weight <- function(X,Y,C,At){
    ...
    # iter = 10
    # for(i in 1:iter){
    #   Lp = L + gamma*sum(Yp*Y<=1)
    #   if(Lp>=L){gamma=gamma-lr}
    #   if(Lp<L){gamma=gamma+lr}
    # }
    alpha_m = gamma*sum(Yp*Y<=1)
    return(alpha_m)
  }
  ...
  l_rate = 0.25 # Learning rate
  num_iter = 200 # Number of iterations in gradient descent
  range = 1 # Range for random values
  n_ensmb = 1 # Number in the ensemble
  ...
  iter = 500
  L_vec = matrix(0,1,(iter+1))
  L_vec[1] = Loss(X,Y,C,At)
  print(L_vec[1])
  for(i in 1:iter){
    Ct = ensemble(X,Y,range,n_ensmb)
    At = Choose(X,Y,C,Ct)
    C_cand = Update_Classifier(X,Y,C,At)
    Yp = predict(X,Y,C_cand)
    L_cand = Loss(X,Y,C_cand,At)
    if(L_cand<L_vec[i]){
      C=C_cand
      L_vec[i+1] = L_cand
      print(L_vec[i+1])
